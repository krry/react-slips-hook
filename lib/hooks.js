...
exports.useSlip = exports.useSlips = exports.useSlipProvider = void 0;
...
function useSlipProvider({ location, processPageQuery, firstPage, pageWidth = 625, obstructedPageWidth = 40, }) {
    const previousFirstPage = react_1.useRef(firstPage);
    const [scroll, containerWidth, setRef, containerRef] = useScroll();
    const [slip, setSlip] = react_1.useState(getRoot(firstPage, processPageQuery));
    ...
    const slipSlugs = react_1.useMemo(() => {
        const res = querystring_1.default.parse(location.search.replace(/^\?/, "")).slip || [];
    ...
    react_1.useEffect(() => {
        ...
        slipSlugs.map((slug) => window.___loader.loadPage(slug))).then((data) => setSlip(getRoot(firstPage, processPageQuery).concat(
        // filter out 404s
        data
            .map((x, i) => ({
            slug: slipSlugs[i],
    ...
    react_1.useEffect(() => {
        const acc = {};
        if (!containerRef.current) {
            setSlipStates(slip.reduce((prev, x, i, a) => {
                prev[x.slug] = {
                    overlay: true,
                    obstructed: false,
                    highlighted: false,
...
exports.useSlipProvider = useSlipProvider;
function useSlips() {
    const { slip, slipStates, navigateToSlip, highlightSlip, } = react_1.useContext(contexts_1.SlipContext);
